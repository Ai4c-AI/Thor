  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.message || t('announcement.createFailed'));
      }
    } catch (error) {
      console.error('Failed to create announcement:', error);
      toast.error(t('announcement.createFailed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={visible} onOpenChange={handleCancel}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t('announcement.createTitle')}</DialogTitle>
          <DialogDescription>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={loading}>
            {loading ? t('announcement.creating') : t('announcement.create')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
