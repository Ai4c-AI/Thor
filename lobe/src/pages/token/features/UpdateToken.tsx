...
export default function UpdateToken({
    ...
    useEffect(() => {
        setInput({
            name: value?.name,
            unlimitedQuota: Boolean(value?.unlimitedQuota),
            remainQuota: value?.remainQuota,
            unlimitedExpired: Boolean(value?.unlimitedExpired),
            expiredTime: value?.expiredTime ?? null,
            limitModels: Array.isArray(value?.limitModels) ? value.limitModels : [],
            whiteIpList: Array.isArray(value?.whiteIpList) ? value.whiteIpList : [],
                ...
                <div style={{
                    ...
                    <Card>
                        <CardContent className="grid gap-4 p-6">
                            <div className="grid gap-1">
                                <Label>{t('token.groups') || '用户组'} *</Label>
                            ...
                            {input.groups.length === 0 && (
                                <p className="text-xs text-red-600">{t('token.selectGroupError') || '请选择至少一个用户组'}</p>
                            )}
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        <CardContent className="grid gap-6 p-6">
                            <div className="grid gap-1">
                                <Label>{t('token.advancedSettings') || '高级设置'}</Label>
                                <p className="text-sm text-muted-foreground">
                                    {t('token.advancedSettingsDesc') || '配置模型限制和IP白名单（可选）'}
                                </p>
                            </div>
                            ...
                            <div className="grid gap-3">
                                <div className="grid gap-1">
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        {t('token.modelLimitDesc') || '限制该Token可以使用的模型，不选择则无限制'}
                                    </p>
                                </div>
                                <div className="grid gap-3">
                            ...
                            <div className="grid gap-3 border-t pt-4">
                                <div className="grid gap-1">
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        {t('token.ipWhitelistDesc') || '限制该Token只能从指定IP地址使用，不设置则无限制'}
                                    </p>
                                </div>
                                <div className="grid gap-3">
